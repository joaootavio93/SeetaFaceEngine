...
void SpatialTransformNet::Execute() {
	...
	LOG(DEBUG) << "Input blobs: (" << num << "," << channels << "," << src_h << "," << src_w << ")";
	...
	for (int n = 0; n < num; ++ n) {
		...
		for (int x = 0; x < dst_h; ++ x)
			...
			for (int c = 0; c < channels; ++ c) {
				if (!is_mat_data_) {
					output_data[output->offset(n, c, x, y)]	= Sampling(input_data + input->offset(n, c), 
						src_h, src_w, src_x, src_y, 1.0 / scale);
				}
				else {
				output_data[output->offset(n, c, x, y)]	= Sampling(reinterpret_cast<unsigned char*>(
...
double SpatialTransformNet::Sampling(const float* const feat_map, int H, int W, double x, double y, double scale) {
	if (type_ == "linear") {
		...
		if (ux >= 0 && ux < H - 1 && uy >= 0 && uy < W - 1) {
			...
			ans = (1 - cof_x) * ans + cof_x * ((1 - cof_y) * feat_map[offset + W] + cof_y * feat_map[offset + W + 1]);
		}
		return ans;
	}
		...
		if (x >= 0 && x < H && y >= 0 && y < W) {
			...
			for (int ux = ceil(x - kernel_width / 2); ux <= floor(x + kernel_width / 2); ++ ux) {
				indices_x.push_back(std::max(std::min(H - 1, ux), 0));
				weights_x.push_back(Cubic((x - ux) * scale));
			}

			// get indices and weight along y axis
			for (int uy = ceil(y - kernel_width / 2); uy <= floor(y + kernel_width / 2); ++ uy) {
...
double SpatialTransformNet::Sampling(const unsigned char* const feat_map, int c, int H, int W, int C, double x, double y, double scale) {
	if (type_ == "linear") {
		// bilinear subsampling
		int ux = floor(x), uy = floor(y);
		double ans = 0;
		if (ux >= 0 && ux < H - 1 && uy >= 0 && uy < W - 1) {
			...
			ans = (1 - cof_x) * ans + cof_x * ((1 - cof_y) * feat_map[offset + W * C] + cof_y * feat_map[offset + W * C + C]);
		}
		return ans;
	}
	else if (type_ == "bicubic") { // Need to be sped up
