...
void TEST(FaceRecognizerTest, ExtractFeature) {
	...
	while (ifs >> img_name >> lb) {
		...
		cv::Mat src_img = cv::imread(test_dir + img_name, 1);
		EXPECT_NE(src_img.data, nullptr) << "Load image error!";
		uint32_t crop_height = face_recognizer.crop_height();
		uint32_t crop_width = face_recognizer.crop_width();
		cv::resize(src_img, src_img, cv::Size(crop_height, crop_width));
		...
		/*for (int i = 0; i < sizeof(feat_sdk); i++) {
			cout << feat_sdk[i] << endl;
		}*/
...
void TEST(FaceRecognizerTest, ExtractFeatureWithCrop) {
	...
	/* Data initialize */
	std::ifstream ifs(test_dir + "crop_sdk_file_list.txt");
	std::string img_name;
	FacialLandmark pt5[5];
