...
void EltwiseNet::Execute() {
	...
	if (op_ == "BAIS_ADDER") {
		...
		LOG(DEBUG) << "input blob: (" << num << "," << input->channels() << "," << height << "," << width << ")";
		LOG(DEBUG) << "bias blob: (" << bias->num() << "," << bias->channels() << "," << bias->height() << "," << bias->width() << ")";
		...
		const Blob* const input = this->input_blobs(0);
		LOG(DEBUG) << "input blob: (" << input->num() << "," << input->channels() << "," << input->height() << ","
			<< input->width() << ")";
		...
		output->CopyData(input->num(), input->channels(), input->height(), input->width(), dst_head);
		delete[] dst_head;
	}
	else if (op_ == "CLOSE") {
		const Blob* const input = this->input_blobs(0);
