	...
	if (num_subnet == 0) {
		...
		for (int i = 0; i < num_subnet; ++ i) {
			if (nets[i]->num_output() > 0) {
				LOG(ERROR) << "There are " << nets[i]->num_output() << " output blobs unlinked!";
				exit(0);
			}
		}
	}
...
void CommonNet::Execute() {
	...
	for (int i = 0; i < input_blobs_.size(); ++ i) {
		...
		for (std::vector<Blob*>::iterator blob = input_plugs_[i].begin(); blob != input_plugs_[i].end(); ++ blob) {
	...
	for (int i = 0; i < output_blobs_.size(); ++ i) {
		...
		for (std::vector<Blob*>::iterator blob = output_plugs_[i].begin(); blob != output_plugs_[i].end(); ++ blob) {
