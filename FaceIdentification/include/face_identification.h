...
namespace seeta {
	...
	public:
		...
		crop_channels().
		SEETA_API uint8_t CropFace(const ImageData &src_image, const FacialLandmark *llpoint, const ImageData &dst_image);

		// Extract feature with a cropping face.
		// 'feats' must be initialized with size of feature_size().
		SEETA_API uint8_t ExtractFeature(const ImageData &crop_image, FaceFeatures const feats);
		...
		SEETA_API uint8_t ExtractFeatureWithCrop(const ImageData &src_image, const FacialLandmark *llpoint,	FaceFeatures const feats);

		// Calculate similarity of face features fc1 and fc2.
		// dim = -1 default feature size
		SEETA_API float CalcSimilarity(FaceFeatures const fc1, FaceFeatures const fc2, long dim = -1);
