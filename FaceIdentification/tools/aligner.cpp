...
namespace seeta {
	...
	Aligner::Aligner(int crop_height, int crop_width, std::string type): crop_height_(crop_height),	crop_width_(crop_width) {
		// build a aligner networks
		net_.reset(new CommonNet());
		HyperParam* common_param = net_->hyper_param();
		common_param->InsertInt("num_subnet", 2);
	...
	void Aligner::Alignment(const ImageData &src_img, const float* const points, Blob* const dst_blob) {
		...
		/*input_data->CopyData(1, src_img.height, src_img.width, src_img.channels, src_img.data);
	...
	void Aligner::Alignment(const ImageData &src_img, const float* const points, const ImageData &dst_img) {
		Blob out_blob;
